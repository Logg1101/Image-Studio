import os
import random
import shutil
from pathlib import Path
from typing import Dict, List, Optional, Tuple

from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QLabel, QTextEdit, QLineEdit,
    QPushButton, QSlider, QSpinBox, QDoubleSpinBox, QComboBox, QGroupBox,
    QProgressBar, QFileDialog, QMessageBox, QFrame, QScrollArea, QSizePolicy,
    QButtonGroup, QSplitter
)
from PySide6.QtCore import Qt, Signal, QRectF
from PySide6.QtGui import QPainter, QColor, QBrush, QPen, QFont

import config.paths as paths
from ui.state import model_manager
from adapters.registry import adapter_registry
from core.types import GenerationRequest, GenerationResult, CharacterRegion, MAX_SEED
from core.prompt_enhancer import AIPromptEnhancer
from engines.sdxl.schedulers import AVAILABLE_SAMPLERS, AVAILABLE_SCHEDULERS
from ui.controllers.generation_controller import GenerationWorker
from ui.widgets.slider_card import SliderCardWidget
from ui.widgets.image_canvas import ImageCanvasWidget
from pipelines.regional_prompting import get_default_regions

# Accent colors for up to 4 characters
CHAR_COLORS = [
    QColor(59, 130, 246),   # Blue
    QColor(16, 185, 129),   # Emerald
    QColor(245, 158, 11),   # Amber
    QColor(168, 85, 247),   # Purple
]

CHAR_COLOR_HEX = ["#3B82F6", "#10B981", "#F59E0B", "#A855F7"]


class CharacterRegionPreviewWidget(QWidget):
    """
    Real-time interactive mini-map visualizing the spatial regions
    of active characters on the canvas.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedHeight(70)
        self.characters_boxes: List[Tuple[float, float, float, float]] = []

    def set_boxes(self, boxes: List[Tuple[float, float, float, float]]):
        self.characters_boxes = boxes
        self.update()

    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)

        # Draw dark canvas background container
        w = self.width() - 8
        h = self.height() - 8
        rect = QRectF(4, 4, w, h)

        painter.setPen(QPen(QColor("#252C3A"), 1.5))
        painter.setBrush(QBrush(QColor("#151A24")))
        painter.drawRoundedRect(rect, 6, 6)

        # Draw character regions
        font = QFont("Segoe UI", 9, QFont.Bold)
        painter.setFont(font)

        for i, box in enumerate(self.characters_boxes):
            x0, y0, x1, y1 = box
            bx = rect.x() + x0 * rect.width()
            by = rect.y() + y0 * rect.height()
            bw = (x1 - x0) * rect.width()
            bh = (y1 - y0) * rect.height()

            color = CHAR_COLORS[i % len(CHAR_COLORS)]
            # Semi-transparent fill
            fill_color = QColor(color.red(), color.green(), color.blue(), 55)
            painter.setBrush(QBrush(fill_color))
            painter.setPen(QPen(color, 1.5, Qt.DashLine if bw < 10 else Qt.SolidLine))

            box_rect = QRectF(bx, by, bw, bh)
            painter.drawRoundedRect(box_rect, 4, 4)

            # Label inside the box
            painter.setPen(QPen(color))
            painter.drawText(box_rect, Qt.AlignCenter, f"Char {i + 1}")


class CharacterCardWidget(QFrame):
    """
    Configurable card for an individual character:
    - Character Prompt
    - Optional LoRA selector & strength
    - Region position controls
    """
    region_changed = Signal()

    def __init__(self, char_idx: int, available_loras: Dict[str, Path], parent=None):
        super().__init__(parent)
        self.char_idx = char_idx
        self.available_loras = available_loras
        self.color_hex = CHAR_COLOR_HEX[char_idx % len(CHAR_COLOR_HEX)]
        self._init_ui()

    def _init_ui(self):
        self.setObjectName("cardPanel")
        self.setStyleSheet(
            f"QFrame#cardPanel {{ background-color: #151A24; border: 1px solid #252C3A; "
            f"border-left: 3px solid {self.color_hex}; border-radius: 8px; padding: 8px; }}"
        )

        lay = QVBoxLayout(self)
        lay.setContentsMargins(6, 6, 6, 6)
        lay.setSpacing(6)

        # Header Row
        hdr = QHBoxLayout()
        lbl = QLabel(f"👤 Character {self.char_idx + 1}")
        lbl.setStyleSheet(f"color: {self.color_hex}; font-weight: 800; font-size: 11px;")
        hdr.addWidget(lbl)
        hdr.addStretch()

        self.region_preset_combo = QComboBox()
        self.region_preset_combo.addItems([
            "Auto Column", "Left Half", "Right Half",
            "Left 1/3", "Center 1/3", "Right 1/3", "Custom"
        ])
        self.region_preset_combo.setStyleSheet("font-size: 10px; max-width: 110px;")
        self.region_preset_combo.currentIndexChanged.connect(self._on_preset_changed)
        hdr.addWidget(QLabel("Region:"))
        hdr.addWidget(self.region_preset_combo)
        lay.addLayout(hdr)

        # Character Prompt Text
        self.prompt_text = QTextEdit()
        self.prompt_text.setPlaceholderText(
            f"Prompt for Character {self.char_idx + 1} (e.g. 1girl, silver hair, red coat, smiling)..."
        )
        self.prompt_text.setFixedHeight(55)
        self.prompt_text.setStyleSheet(
            "background-color: #0F131B; border: 1px solid #252C3A; border-radius: 6px; "
            "color: #E8ECF4; font-size: 11px; padding: 4px;"
        )
        lay.addWidget(self.prompt_text)

        # Controls Row: LoRA Selector & Strength
        ctrl_row = QHBoxLayout()
        ctrl_row.setSpacing(6)

        ctrl_row.addWidget(QLabel("LoRA:"))
        self.lora_combo = QComboBox()
        self.lora_combo.addItem("None", None)
        for name in sorted(self.available_loras.keys()):
            self.lora_combo.addItem(f"🧩 {name}", name)
        self.lora_combo.setStyleSheet("font-size: 10px;")
        ctrl_row.addWidget(self.lora_combo, stretch=2)

        ctrl_row.addWidget(QLabel("Weight:"))
        self.lora_strength = QDoubleSpinBox()
        self.lora_strength.setRange(-2.0, 2.0)
        self.lora_strength.setSingleStep(0.05)
        self.lora_strength.setValue(0.80)
        self.lora_strength.setFixedWidth(58)
        self.lora_strength.setStyleSheet("font-size: 10px;")
        ctrl_row.addWidget(self.lora_strength)

        lay.addLayout(ctrl_row)

        # Custom Region Coordinate Sliders (Hidden unless "Custom" is selected)
        self.custom_box_frame = QFrame()
        custom_lay = QHBoxLayout(self.custom_box_frame)
        custom_lay.setContentsMargins(0, 0, 0, 0)
        custom_lay.setSpacing(6)

        custom_lay.addWidget(QLabel("X0:"))
        self.x0_spin = QSpinBox()
        self.x0_spin.setRange(0, 100)
        self.x0_spin.setValue(0)
        self.x0_spin.setSuffix("%")
        custom_lay.addWidget(self.x0_spin)

        custom_lay.addWidget(QLabel("X1:"))
        self.x1_spin = QSpinBox()
        self.x1_spin.setRange(0, 100)
        self.x1_spin.setValue(50)
        self.x1_spin.setSuffix("%")
        custom_lay.addWidget(self.x1_spin)

        self.x0_spin.valueChanged.connect(lambda _: self.region_changed.emit())
        self.x1_spin.valueChanged.connect(lambda _: self.region_changed.emit())

        self.custom_box_frame.setVisible(False)
        lay.addWidget(self.custom_box_frame)

    def _on_preset_changed(self, idx: int):
        preset = self.region_preset_combo.currentText()
        self.custom_box_frame.setVisible(preset == "Custom")
        self.region_changed.emit()

    def set_available_loras(self, available: Dict[str, Path]):
        self.available_loras = available
        curr = self.lora_combo.currentText()
        self.lora_combo.clear()
        self.lora_combo.addItem("None", None)
        for name in sorted(available.keys()):
            self.lora_combo.addItem(f"🧩 {name}", name)
        # Restore if still present
        idx = self.lora_combo.findText(curr)
        if idx >= 0:
            self.lora_combo.setCurrentIndex(idx)

    def set_region_box(self, box: Tuple[float, float, float, float]):
        x0, y0, x1, y1 = box
        self.x0_spin.blockSignals(True)
        self.x1_spin.blockSignals(True)
        self.x0_spin.setValue(int(round(x0 * 100)))
        self.x1_spin.setValue(int(round(x1 * 100)))
        self.x0_spin.blockSignals(False)
        self.x1_spin.blockSignals(False)

    def get_region_box(self) -> Tuple[float, float, float, float]:
        preset = self.region_preset_combo.currentText()
        if preset == "Left Half":
            return (0.0, 0.0, 0.5, 1.0)
        elif preset == "Right Half":
            return (0.5, 0.0, 1.0, 1.0)
        elif preset == "Left 1/3":
            return (0.0, 0.0, 0.3333, 1.0)
        elif preset == "Center 1/3":
            return (0.3333, 0.0, 0.6667, 1.0)
        elif preset == "Right 1/3":
            return (0.6667, 0.0, 1.0, 1.0)
        elif preset == "Custom":
            x0 = self.x0_spin.value() / 100.0
            x1 = self.x1_spin.value() / 100.0
            return (min(x0, x1), 0.0, max(x0, x1), 1.0)
        return (0.0, 0.0, 1.0, 1.0)

    def to_character_region(self, default_box: Tuple[float, float, float, float]) -> CharacterRegion:
        preset = self.region_preset_combo.currentText()
        if preset == "Auto Column":
            box = default_box
        else:
            box = self.get_region_box()

        lora_name = self.lora_combo.currentData()
        return CharacterRegion(
            prompt=self.prompt_text.toPlainText().strip(),
            lora_name=lora_name,
            lora_strength=float(self.lora_strength.value()),
            box=box,
            feather=0.04
        )


class MultiCharacterPage(QWidget):
    """
    Dedicated Multi-Character Studio.
    Synthesizes 2 to 4 distinct characters with independent regional prompts,
    independent LoRAs, and isolated conditioning to avoid attribute bleeding.
    """
    send_to_editor_requested = Signal(str)
    send_to_i2i_requested = Signal(str)

    def __init__(self, parent=None):
        super().__init__(parent)
        self.worker: Optional[GenerationWorker] = None
        self.neg_prompt_visible = False
        self.character_cards: List[CharacterCardWidget] = []
        self._init_ui()

    def _init_ui(self):
        main_layout = QHBoxLayout(self)
        main_layout.setContentsMargins(6, 6, 6, 6)
        main_layout.setSpacing(10)

        # ==========================================================
        # 1. LEFT SIDEBAR PANEL (Scrollable independently, ~350px)
        # ==========================================================
        scroll_area = QScrollArea()
        scroll_area.setWidgetResizable(True)
        scroll_area.setFixedWidth(350)
        scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)

        sidebar_content = QWidget()
        sidebar_lay = QVBoxLayout(sidebar_content)
        sidebar_lay.setContentsMargins(4, 4, 8, 4)
        sidebar_lay.setSpacing(10)

        # Section 1: ACTIVE CHECKPOINT
        ckpt_group = QFrame()
        ckpt_group.setObjectName("cardPanel")
        ckpt_lay = QVBoxLayout(ckpt_group)
        ckpt_lay.setContentsMargins(8, 8, 8, 8)
        ckpt_lay.setSpacing(6)

        ckpt_header = QHBoxLayout()
        ckpt_lbl = QLabel("🧠 ACTIVE CHECKPOINT")
        ckpt_lbl.setStyleSheet("color: #8993A7; font-weight: 800; font-size: 11px; letter-spacing: 0.5px;")
        self.status_dot = QLabel("● Ready")
        self.status_dot.setStyleSheet("color: #10B981; font-size: 10px; font-weight: bold;")
        ckpt_header.addWidget(ckpt_lbl)
        ckpt_header.addStretch()
        ckpt_header.addWidget(self.status_dot)
        ckpt_lay.addLayout(ckpt_header)

        ckpt_row = QHBoxLayout()
        self.model_combo = QComboBox()
        self.refresh_models_btn = QPushButton("🔄")
        self.refresh_models_btn.setFixedSize(30, 30)
        self.refresh_models_btn.setToolTip("Rescan Checkpoints")
        ckpt_row.addWidget(self.model_combo, stretch=1)
        ckpt_row.addWidget(self.refresh_models_btn)
        ckpt_lay.addLayout(ckpt_row)
        sidebar_lay.addWidget(ckpt_group)

        # Section 2: ASPECT RATIO & DIMENSIONS
        ar_group = QFrame()
        ar_group.setObjectName("cardPanel")
        ar_lay = QVBoxLayout(ar_group)
        ar_lay.setContentsMargins(8, 8, 8, 8)
        ar_lay.setSpacing(6)

        ar_header = QLabel("📐 ASPECT RATIO & RESOLUTION")
        ar_header.setStyleSheet("color: #8993A7; font-weight: 800; font-size: 11px; letter-spacing: 0.5px;")
        ar_lay.addWidget(ar_header)

        ar_btn_row = QHBoxLayout()
        ar_btn_row.setSpacing(4)
        self.ar_buttons = {}
        # Default landscape 16:9 for multi-character works exceptionally well
        ratios = [("16:9", 1344, 768), ("1:1", 1024, 1024), ("9:16", 768, 1344), ("4:3", 1152, 896), ("3:4", 896, 1152)]
        for label, w, h in ratios:
            btn = QPushButton(label)
            btn.setObjectName("aspectBtn")
            btn.setCheckable(True)
            if label == "16:9":
                btn.setChecked(True)
            btn.clicked.connect(lambda _, w=w, h=h, l=label: self._on_aspect_clicked(l, w, h))
            ar_btn_row.addWidget(btn)
            self.ar_buttons[label] = btn
        ar_lay.addLayout(ar_btn_row)

        self.width_card = SliderCardWidget("Width (px)", 512, 2048, 1344, step=64)
        self.height_card = SliderCardWidget("Height (px)", 512, 2048, 768, step=64)
        ar_lay.addWidget(self.width_card)
        ar_lay.addWidget(self.height_card)
        sidebar_lay.addWidget(ar_group)

        # Section 3: SAMPLING PARAMETERS
        samp_group = QFrame()
        samp_group.setObjectName("cardPanel")
        samp_lay = QVBoxLayout(samp_group)
        samp_lay.setContentsMargins(8, 8, 8, 8)
        samp_lay.setSpacing(6)

        samp_header = QLabel("🎛️ SAMPLING PARAMETERS")
        samp_header.setStyleSheet("color: #8993A7; font-weight: 800; font-size: 11px; letter-spacing: 0.5px;")
        samp_lay.addWidget(samp_header)

        self.steps_card = SliderCardWidget("Sampling Steps", 10, 60, 28)
        self.cfg_card = SliderCardWidget("CFG Guidance", 1, 15, 7)
        samp_lay.addWidget(self.steps_card)
        samp_lay.addWidget(self.cfg_card)

        samp_sched_row = QHBoxLayout()
        s_box = QVBoxLayout()
        s_box.setSpacing(2)
        s_lbl = QLabel("Sampler:")
        s_lbl.setStyleSheet("color: #8993A7; font-size: 10px; font-weight: 600;")
        self.sampler_combo = QComboBox()
        self.sampler_combo.addItems(AVAILABLE_SAMPLERS)
        s_box.addWidget(s_lbl)
        s_box.addWidget(self.sampler_combo)
        samp_sched_row.addLayout(s_box)

        sc_box = QVBoxLayout()
        sc_box.setSpacing(2)
        sc_lbl = QLabel("Schedule:")
        sc_lbl.setStyleSheet("color: #8993A7; font-size: 10px; font-weight: 600;")
        self.sched_combo = QComboBox()
        self.sched_combo.addItems(AVAILABLE_SCHEDULERS)
        sc_box.addWidget(sc_lbl)
        sc_box.addWidget(self.sched_combo)
        samp_sched_row.addLayout(sc_box)
        samp_lay.addLayout(samp_sched_row)
        sidebar_lay.addWidget(samp_group)

        # Section 4: HIGH-RES UPSCALER
        up_group = QFrame()
        up_group.setObjectName("cardPanel")
        up_lay = QVBoxLayout(up_group)
        up_lay.setContentsMargins(8, 8, 8, 8)
        up_lay.setSpacing(6)

        up_header = QLabel("🔍 HIGH-RES UPSCALER")
        up_header.setStyleSheet("color: #8993A7; font-weight: 800; font-size: 11px; letter-spacing: 0.5px;")
        up_lay.addWidget(up_header)

        up_row = QHBoxLayout()
        up_m_box = QVBoxLayout()
        up_m_box.setSpacing(2)
        up_m_lbl = QLabel("Method:")
        up_m_lbl.setStyleSheet("color: #8993A7; font-size: 10px; font-weight: 600;")
        self.upscale_combo = QComboBox()
        self.upscale_combo.addItems(["None", "4x-RealCUGAN", "Tiled SDXL", "Lanczos", "Bicubic"])
        up_m_box.addWidget(up_m_lbl)
        up_m_box.addWidget(self.upscale_combo)
        up_row.addLayout(up_m_box)

        up_s_box = QVBoxLayout()
        up_s_box.setSpacing(2)
        up_s_lbl = QLabel("Scale:")
        up_s_lbl.setStyleSheet("color: #8993A7; font-size: 10px; font-weight: 600;")
        self.scale_factor_combo = QComboBox()
        self.scale_factor_combo.addItems(["1.5x", "2.0x", "3.0x", "4.0x"])
        self.scale_factor_combo.setCurrentIndex(1)
        up_s_box.addWidget(up_s_lbl)
        up_s_box.addWidget(self.scale_factor_combo)
        up_row.addLayout(up_s_box)
        up_lay.addLayout(up_row)
        sidebar_lay.addWidget(up_group)

        sidebar_lay.addStretch()
        scroll_area.setWidget(sidebar_content)
        main_layout.addWidget(scroll_area, stretch=0)

        # ==========================================================
        # 2. CENTER & RIGHT WORKSPACE PANELS
        # ==========================================================
        workspace = QFrame()
        workspace.setObjectName("workspacePanel")
        workspace_lay = QVBoxLayout(workspace)
        workspace_lay.setContentsMargins(10, 10, 10, 10)
        workspace_lay.setSpacing(8)

        inner_row = QHBoxLayout()
        inner_row.setSpacing(12)

        # --- LEFT INNER: Multi-Character Prompting Panel ---
        left_inner = QVBoxLayout()
        left_inner.setSpacing(8)

        # A. Global Prompt Header
        p_hdr_row = QHBoxLayout()
        p_title = QLabel("🌐 GLOBAL SCENE PROMPT (Environment & Lighting)")
        p_title.setStyleSheet("font-weight: 800; font-size: 11px; color: #E8ECF4; letter-spacing: 0.5px;")
        self.token_label = QLabel("~0 tokens (0 chars)")
        self.token_label.setStyleSheet(
            "background: #151A24; border: 1px solid #252C3A; border-radius: 4px; "
            "padding: 2px 8px; font-family: 'JetBrains Mono', monospace; font-size: 10px; color: #8993A7;"
        )
        p_hdr_row.addWidget(p_title)
        p_hdr_row.addStretch()
        p_hdr_row.addWidget(self.token_label)
        left_inner.addLayout(p_hdr_row)

        self.global_prompt_text = QTextEdit()
        self.global_prompt_text.setPlaceholderText(
            "Describe the overall scene, background, mood, lighting, and composition (shared across all characters)..."
        )
        self.global_prompt_text.setText(
            "masterpiece, highly detailed, fantasy garden with ancient stone ruins, blooming wisteria, golden hour sunlight, volumetric god rays"
        )
        self.global_prompt_text.setFixedHeight(65)
        left_inner.addWidget(self.global_prompt_text)

        # Global Prompt Tools Row
        tools_row = QHBoxLayout()
        tools_row.setSpacing(6)
        self.enhance_btn = QPushButton("✨ AI Enhance")
        self.rand_prompt_btn = QPushButton("🎲 Randomize")
        self.clear_prompt_btn = QPushButton("🧹 Clear")
        for b in (self.enhance_btn, self.rand_prompt_btn, self.clear_prompt_btn):
            b.setStyleSheet("font-size: 11px; padding: 4px 8px;")
        tools_row.addWidget(self.enhance_btn)
        tools_row.addWidget(self.rand_prompt_btn)
        tools_row.addWidget(self.clear_prompt_btn)
        tools_row.addStretch()
        left_inner.addLayout(tools_row)

        # Negative Prompt Collapsible
        self.neg_toggle_btn = QPushButton("Negative Prompt  ▼")
        self.neg_toggle_btn.setStyleSheet(
            "text-align: left; background: #151A24; border: 1px solid #252C3A; "
            "border-radius: 6px; padding: 4px 8px; color: #8993A7; font-weight: 600;"
        )
        left_inner.addWidget(self.neg_toggle_btn)

        self.neg_prompt_text = QTextEdit()
        self.neg_prompt_text.setPlaceholderText("Negative prompt to suppress unwanted artifacts...")
        self.neg_prompt_text.setText("blurry, low quality, distorted anatomy, duplicate characters, fused bodies, extra limbs, bad hands")
        self.neg_prompt_text.setFixedHeight(50)
        self.neg_prompt_text.setVisible(False)
        left_inner.addWidget(self.neg_prompt_text)

        # B. Character Count Selector Row
        count_row = QHBoxLayout()
        count_lbl = QLabel("👥 CHARACTERS ON CANVAS:")
        count_lbl.setStyleSheet("font-weight: 800; font-size: 11px; color: #E8ECF4;")
        count_row.addWidget(count_lbl)

        self.count_btn_group = QButtonGroup(self)
        self.count_buttons = {}
        for count in (2, 3, 4):
            btn = QPushButton(f"{count} Characters")
            btn.setCheckable(True)
            if count == 2:
                btn.setChecked(True)
            btn.setStyleSheet(
                "QPushButton { background: #151A24; border: 1px solid #252C3A; border-radius: 6px; padding: 5px 12px; font-weight: 600; } "
                "QPushButton:checked { background: #2563EB; border-color: #3B82F6; color: white; }"
            )
            btn.clicked.connect(lambda _, c=count: self._set_character_count(c))
            self.count_btn_group.addButton(btn)
            self.count_buttons[count] = btn
            count_row.addWidget(btn)

        count_row.addStretch()
        left_inner.addLayout(count_row)

        # C. Visual Layout Mini-Map Preview
        self.preview_mini_map = CharacterRegionPreviewWidget()
        left_inner.addWidget(self.preview_mini_map)

        # D. Scrollable Character Cards Stack
        cards_scroll = QScrollArea()
        cards_scroll.setWidgetResizable(True)
        cards_scroll.setStyleSheet("background: transparent; border: none;")

        self.cards_container = QWidget()
        self.cards_lay = QVBoxLayout(self.cards_container)
        self.cards_lay.setContentsMargins(0, 0, 0, 0)
        self.cards_lay.setSpacing(8)

        cards_scroll.setWidget(self.cards_container)
        left_inner.addWidget(cards_scroll, stretch=1)

        # E. Primary Action Bar (Generate & Stop)
        action_row = QHBoxLayout()
        action_row.setSpacing(8)
        self.gen_btn = QPushButton("👥⚡ Generate Multi-Character")
        self.gen_btn.setObjectName("btnGenerate")
        self.stop_btn = QPushButton("🛑 Stop")
        self.stop_btn.setObjectName("btnStop")
        self.stop_btn.setEnabled(False)

        action_row.addWidget(self.gen_btn, stretch=3)
        action_row.addWidget(self.stop_btn, stretch=1)
        left_inner.addLayout(action_row)

        inner_row.addLayout(left_inner, stretch=3)

        # --- RIGHT INNER: Output Canvas ---
        right_inner = QVBoxLayout()
        right_inner.setSpacing(6)

        out_hdr = QHBoxLayout()
        out_badge = QLabel("🖼️ Render Output Canvas")
        out_badge.setStyleSheet(
            "background: #151A24; border: 1px solid #252C3A; color: #4F9CFF; "
            "border-radius: 6px; padding: 4px 10px; font-weight: 800; font-size: 11px;"
        )
        out_hdr.addWidget(out_badge)
        out_hdr.addStretch()

        self.save_icon_btn = QPushButton("💾 Save Image")
        self.save_icon_btn.setStyleSheet("font-size: 11px; padding: 4px 10px;")
        out_hdr.addWidget(self.save_icon_btn)
        right_inner.addLayout(out_hdr)

        self.canvas = ImageCanvasWidget()
        right_inner.addWidget(self.canvas, stretch=1)

        self.status_bar_lbl = QLabel("✓ Ready to generate")
        self.status_bar_lbl.setAlignment(Qt.AlignCenter)
        self.status_bar_lbl.setStyleSheet(
            "background: #151A24; border: 1px solid #252C3A; border-radius: 6px; "
            "color: #10B981; font-family: 'JetBrains Mono', monospace; font-size: 11px; padding: 6px 12px;"
        )
        right_inner.addWidget(self.status_bar_lbl)

        self.progress_bar = QProgressBar()
        self.progress_bar.setFixedHeight(14)
        self.progress_bar.setVisible(False)
        right_inner.addWidget(self.progress_bar)

        send_row = QHBoxLayout()
        send_row.setSpacing(6)
        self.send_i2i_btn = QPushButton("↗️ Send to Img2Img")
        self.send_i2i_btn.setObjectName("btnSecondary")
        self.send_editor_btn = QPushButton("⚗️ Send to Alchemy")
        self.send_editor_btn.setObjectName("btnSecondary")
        send_row.addWidget(self.send_i2i_btn)
        send_row.addWidget(self.send_editor_btn)
        right_inner.addLayout(send_row)

        inner_row.addLayout(right_inner, stretch=3)
        workspace_lay.addLayout(inner_row)

        main_layout.addWidget(workspace, stretch=1)

        # Populate Initial 2 Character Cards
        self._build_character_cards(2)

        # Wire Signals
        self._refresh_models()
        self.refresh_models_btn.clicked.connect(self._refresh_models)
        self.global_prompt_text.textChanged.connect(self._update_tokens)
        self.enhance_btn.clicked.connect(self._enhance_global_prompt)
        self.rand_prompt_btn.clicked.connect(self._randomize_prompt)
        self.clear_prompt_btn.clicked.connect(self.global_prompt_text.clear)
        self.neg_toggle_btn.clicked.connect(self._toggle_neg_prompt)
        self.gen_btn.clicked.connect(self._start_generation)
        self.stop_btn.clicked.connect(self._cancel_generation)
        self.save_icon_btn.clicked.connect(self._save_image)
        self.send_i2i_btn.clicked.connect(self._send_to_i2i)
        self.send_editor_btn.clicked.connect(self._send_to_editor)
        self._update_tokens()

    def _refresh_models(self):
        self.model_combo.clear()
        for name in model_manager.available_models.keys():
            self.model_combo.addItem(name)
        scanned_loras = adapter_registry.scan_loras()
        for card in self.character_cards:
            card.set_available_loras(scanned_loras)

    def _on_aspect_clicked(self, label: str, w: int, h: int):
        for l, btn in self.ar_buttons.items():
            btn.setChecked(l == label)
        self.width_card.setValue(w)
        self.height_card.setValue(h)

    def _update_tokens(self):
        txt = self.global_prompt_text.toPlainText()
        chars = len(txt)
        tokens = int(len(txt.split()) * 1.3)
        self.token_label.setText(f"~{tokens} tokens ({chars} chars)")

    def _enhance_global_prompt(self):
        cur = self.global_prompt_text.toPlainText().strip()
        enhanced = AIPromptEnhancer.enhance(cur)
        self.global_prompt_text.setText(enhanced)

    def _randomize_prompt(self):
        ideas = [
            "masterpiece, ancient high-fantasy courtyard under twin full moons, ethereal glowing lotus flowers, cinematic atmospheric lighting",
            "cyberpunk neo-tokyo rooftop at night, glowing holographic billboards, gentle rain, reflective neon puddles",
            "cozy medieval tavern interior with a warm stone fireplace, rustic wooden tables, soft ambient candlelight, warm haze"
        ]
        self.global_prompt_text.setText(random.choice(ideas))

    def _toggle_neg_prompt(self):
        self.neg_prompt_visible = not self.neg_prompt_visible
        self.neg_prompt_text.setVisible(self.neg_prompt_visible)
        self.neg_toggle_btn.setText("Negative Prompt  ▲" if self.neg_prompt_visible else "Negative Prompt  ▼")

    def _set_character_count(self, count: int):
        if count == len(self.character_cards):
            return
        self._build_character_cards(count)

    def _build_character_cards(self, count: int):
        # Clear existing cards
        for card in self.character_cards:
            self.cards_lay.removeWidget(card)
            card.deleteLater()
        self.character_cards.clear()

        scanned_loras = adapter_registry.scan_loras()
        defaults = get_default_regions(count)

        # Default sample prompts for instant usability
        sample_prompts = [
            "1girl, silver hair, ponytail, emerald eyes, white royal sorceress robe, gold ornaments, holding glowing staff, standing elegantly",
            "1girl, long dark crimson hair, amber eyes, black gothic knight armor, ornate greatsword on back, confident expression",
            "1boy, messy brown hair, blue eyes, adventurer tunic, leather armor, smiling friendly",
            "1girl, blonde twin braids, violet eyes, celestial priestess gown, floating ribbon, gentle smile"
        ]

        for i in range(count):
            card = CharacterCardWidget(i, scanned_loras)
            card.set_region_box(defaults[i])
            if i < len(sample_prompts):
                card.prompt_text.setText(sample_prompts[i])
            card.region_changed.connect(self._update_preview_minimap)
            self.cards_lay.addWidget(card)
            self.character_cards.append(card)

        self._update_preview_minimap()

    def _update_preview_minimap(self):
        count = len(self.character_cards)
        defaults = get_default_regions(count)
        boxes = []
        for i, card in enumerate(self.character_cards):
            preset = card.region_preset_combo.currentText()
            if preset == "Auto Column":
                boxes.append(defaults[i])
            else:
                boxes.append(card.get_region_box())
        self.preview_mini_map.set_boxes(boxes)

    def _start_generation(self):
        selected_model_name = self.model_combo.currentText()
        if not selected_model_name or selected_model_name not in model_manager.available_models:
            QMessageBox.warning(self, "Model Required", "Please select an available checkpoint.")
            return

        model_info = model_manager.available_models[selected_model_name]
        count = len(self.character_cards)
        defaults = get_default_regions(count)

        characters: List[CharacterRegion] = []
        for i, card in enumerate(self.character_cards):
            char_reg = card.to_character_region(defaults[i])
            characters.append(char_reg)

        global_prompt = self.global_prompt_text.toPlainText().strip() or "masterpiece, high quality"
        neg_prompt = self.neg_prompt_text.toPlainText().strip() if self.neg_prompt_visible else None

        req = GenerationRequest(
            model=model_info,
            prompt=global_prompt,
            negative_prompt=neg_prompt,
            characters=characters,
            width=self.width_card.getValue(),
            height=self.height_card.getValue(),
            steps=self.steps_card.getValue(),
            guidance_scale=float(self.cfg_card.getValue()),
            sampler=self.sampler_combo.currentText(),
            scheduler=self.sched_combo.currentText(),
            seed=random.randint(0, MAX_SEED),
            upscale_method=self.upscale_combo.currentText(),
            upscale_factor=float(self.scale_factor_combo.currentText().replace("x", ""))
        )

        self.progress_bar.setVisible(True)
        self.gen_btn.setEnabled(False)
        self.stop_btn.setEnabled(True)
        self.status_bar_lbl.setText("⚙️ Initializing multi-character regional pipeline...")

        self.worker = GenerationWorker(req)
        self.worker.progress_changed.connect(self._on_progress)
        self.worker.finished_success.connect(self._on_success)
        self.worker.finished_error.connect(self._on_error)
        self.worker.cancelled.connect(self._on_cancelled)
        self.worker.start()

    def _on_progress(self, step: int, total: int, desc: str):
        pct = int((step / max(total, 1)) * 100)
        self.progress_bar.setValue(pct)
        self.progress_bar.setFormat(f"{desc} ({pct}%)")
        self.status_bar_lbl.setText(f"Multi-Character Sampling: Step {step}/{total} ({pct}%)")

    def _on_success(self, res: GenerationResult):
        self.canvas.set_image(res.image_path)
        self._reset_ui()
        self.status_bar_lbl.setText(f"✓ Render complete in {res.generation_time_ms / 1000:.2f}s")
        self.progress_bar.setVisible(False)

    def _on_error(self, err: str):
        QMessageBox.critical(self, "Generation Error", err)
        self._reset_ui()
        self.status_bar_lbl.setText(f"⚠️ Error: {err}")
        self.progress_bar.setVisible(False)

    def _on_cancelled(self):
        self._reset_ui()
        self.status_bar_lbl.setText("Generation cancelled by user.")
        self.progress_bar.setVisible(False)

    def _cancel_generation(self):
        if self.worker and self.worker.isRunning():
            self.worker.cancel()

    def _reset_ui(self):
        self.gen_btn.setEnabled(True)
        self.stop_btn.setEnabled(False)

    def _save_image(self):
        if not self.canvas.current_image_path:
            return
        dest, _ = QFileDialog.getSaveFileName(self, "Save Multi-Character Image", "multi_character.png", "PNG Images (*.png)")
        if dest:
            shutil.copy(self.canvas.current_image_path, dest)

    def _send_to_i2i(self):
        if self.canvas.current_image_path:
            self.send_to_i2i_requested.emit(self.canvas.current_image_path)

    def _send_to_editor(self):
        if self.canvas.current_image_path:
            self.send_to_editor_requested.emit(self.canvas.current_image_path)
