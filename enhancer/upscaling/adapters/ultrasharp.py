...
class UltraSharpAdapter(BaseUpscalerAdapter):
    """
    Neural 4x-UltraSharp RRDBNet Upscaler Adapter.
    Specialized for high-frequency edge recovery, line art clarity, and photo reconstruction.
    """

    def __init__(self, model_path: Optional[str] = None):
        if model_path is None:
            from config.paths import UPSCALERS_DIR
            self._model_path = str((UPSCALERS_DIR / "4x-UltraSharp.pth").resolve())
        else:
    ...
    @property
    def name(self) -> str:
        return "4x-UltraSharp (Neural ESRGAN)"
    ...
    def is_available(self) -> bool:
        if os.path.exists(self._model_path):
            return True
        from config.paths import UPSCALERS_DIR
        alt_path = UPSCALERS_DIR / "4x-UltraSharp.pth"
        if alt_path.exists():
    ...
    def upscale(self, image_tensor: torch.Tensor, scale: int = 4) -> torch.Tensor:
        """
        Upscales an input tensor [1, 3, H, W] in [0.0, 1.0].
        """
        ...
        # Scale down if 1x or 2x requested
